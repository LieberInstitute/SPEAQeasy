...
for samp in $samp_names; do
    ...
    #  Grab all unique process submissions for this sample
    proc_lines=$(grep "\[.*/.*\] process.*\> ($samp)" $LOG | cut -d "(" -f 1 | awk '!seen[$0]++')
    
    #  Loop through these
    for line_num in `seq 1 $(echo "$proc_lines" | wc -l)`; do
        proc_line=$(echo "$proc_lines" | awk "NR==$line_num")
        
        #  Infer process name, working directory, and exit code from the log
