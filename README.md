...
+ Additional software configuration depends on the options available on your system/ execution environment:
    ...
    + **Installing dependencies locally** (Recommended for non-JHPCE users): The script `install_software.sh` is included in the repository, and automates the installation process. Make sure that you first just have Java (8 or later) and Python 2.7 installed globally (requiring root privileges). Afterward, open `install_software.sh` and change line 10 to read `local_install=true`. Then run the script to install all software dependencies locally, via `bash install_software.sh`, from inside the repository.
    + **Using docker** (Experimental): If docker is installed in your environment, this option requires no local installation of software. Simply add the option `-profile docker` to the main script (this is determined at step 4 in the section you choose below). The required containers are pulled at run-time automatically. You will also need to run `bash install_software.sh` from inside the repository. This installs nextflow locally, and R packages (in a default location) for use in the pipeline.
