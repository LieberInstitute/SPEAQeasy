...
**Alignment SAMs, BAMs, and Summaries** `alignment/`

- `[sample_name]_hisat_out.sam`: The main alignment output from [Hisat2](http://daehwankimlab.github.io/hisat2/), if used, in SAM format.
- `[sample_name].sam`: The main alignment output from [STAR](https://github.com/alexdobin/STAR), if used, in SAM format.
...
- sam_to_bam/[sample_name]_accepted_hits.sorted.bam` and `sam_to_bam/[sample_name]_accepted_hits.sorted.bam.bai`: Alignments where unmapped reads are filtered out, and the resulting BAM file is coordinate-sorted and indexed.
