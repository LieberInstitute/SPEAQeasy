...
**Note:** users at the JHPCE cluster can use the existing installation at `/dcl01/lieber/ajaffe/Nick/SPEAQeasy/`, skipping the above process. Instead, simply make sure you have the required R packages installed before running the pipeline:

```{bash, eval=FALSE}
#  Load the current stable R module
module load conda_R

#  Install required R packages
Rscript /dcl01/lieber/ajaffe/Nick/SPEAQeasy/scripts/check_R_packages_JHPCE.R
...
- *required utilities are missing*: this is particularly common issue for MacOS users, or Linux users trying to get SPEAQeasy running on a local machine (like a laptop). In either case, we will assume the user has root privileges for the solutions suggested below. 

```{bash, eval=FALSE}
install_software.sh: line 99: wget: command not found
```

Mac OS users in particular may be missing the utility `wget`, required for the SPEAQeasy installation. A solution on Mac is to install [brew](https://brew.sh/), and install `wget` through `brew`:
...
It is recommended that Linux users install some basic dependencies if local installation fails for any reason.
...
- *docker permissions issues*: users managing dependencies with docker might encounter error messages if docker is not properly configured:
...
As noted in the [installation](#Installation) section, it is recommended that you use the existing JHPCE installation at `/dcl01/lieber/ajaffe/Nick/SPEAQeasy/`. The following directions describe how to do this.

- (Optional) **Use custom configuration**: default configuration with thoroughly testing hardware resource specification is described within `/dcl01/lieber/ajaffe/Nick/SPEAQeasy/conf/jhpce.config`. If you wish to use any different settings than those provided, feel free to create your own configuration file under `/dcl01/lieber/ajaffe/Nick/SPEAQeasy/conf/`. More detailed instructions are provided [here](#sharing).
- **Copy the main script, modify, and run**: the original "main" script is `/dcl01/lieber/ajaffe/Nick/SPEAQeasy/run_pipeline_jhpce_qsub.sh`. Please copy this script to a directory of your choice. From this directory, the pipeline run is submitted to the cluster by executing `qsub run_pipeline_jhpce.sh`. See the [full list of command-line options](#command-opts) and [section on sharing pipeline installations](#sharing) for details and tips about modifying your copy of `run_pipeline_jhpce.sh`.
...
After the main call, `nextflow /dcl01/lieber/ajaffe/Nick/SPEAQeasy/main.nf`, each command option can be described line by line:
...
#$ -o ./run_brain_subset.log
#$ -e ./run_brain_subset.log
#$ -cwd
...
bash /dcl01/lieber/ajaffe/Nick/SPEAQeasy/scripts/generate_logs.sh $PWD/run_brain_subset.log
```
