...
- **Modify the main script and run**: the "main" script is `run_pipeline_jhpce_qsub.sh`. The pipeline run is submitted as a job to the cluster by executing `qsub run_pipeline_jhpce.sh`. See the [full list of command-line options](#command-opts) for other details about modifying the script for your use-case.
